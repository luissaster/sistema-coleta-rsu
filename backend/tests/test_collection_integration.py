...
Movido para evitar conflito com o módulo 'collections' nativo do Python
"""
import pytest
...
class TestCollectionFlow:
    ...
    def test_complete_collection_flow(self):
        ...
        assert self.collection.start_time is not None
        ...
        assert self.collection.collection_items.count() == 2
        ...
        assert self.collection.status == 'completed'
        assert self.collection.total_weight == Decimal('450.0')
        assert self.collection.points_completed == 2
    ...
    def test_partial_collection(self):
        ...
        assert self.collection.points_completed == 1
        assert self.collection.total_points == 2
    ...
    def test_collection_cancellation(self):
        ...
        assert self.collection.status == 'cancelled'
        assert 'quebrou' in self.collection.notes
...
class TestRouteExecution:
    ...
    def test_route_execution_lifecycle(self):
        ...
        assert self.execution.actual_end_time is not None
